...
- 支持同时加载多个模型：每个模型可单独设置类别偏移、置信度阈值、类别过滤，预测时自动合并结果
- 多模型去重（NMS）：同一目标被多个模型重复检出时，同类别只保留置信度最高的框；不同类别（如同一个人身上的安全帽 + 反光衣）会同时保留
...
1. 配置里加载模型（可多个）
2. 点“🤖 自动化标注”
3. 弹窗中设置“仅标注未标注的图片”、去重 IoU 阈值
4. 点“开始自动化标注”，弹窗内实时显示进度条、当前文件、已标注 / 跳过数量
...
1. 配置 → 点“+ 添加模型”，填写 `.pt` 路径（如 `weights/best.pt`）→ 点“加载模型”
2. 可继续“+ 添加模型”加载第二个模型（如安全帽模型 + 反光衣模型），分别设置类别偏移和置信度阈值
3. 每个模型加载后勾选“只标注以下模型类别”（默认全选；全部不勾 = 不输出任何框）
4. 设置全局去重 IoU 阈值（默认 0.5）
5. 点“✨ 模型预测”，人工确认 / 修正后保存
...
工具已内置跨模型 NMS：同类别重叠框只会保留置信度最高的一个；不同类别（如安全帽 + 反光衣）会同时保留。可以在配置里调低“全局去重 IoU 阈值”让去重更严格。
